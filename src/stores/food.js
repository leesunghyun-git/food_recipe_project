...
/*
        ...
        => state가 변동이 되면 자동으로 화면 UI가 자동 업데이트
    ...
    action : 비동기 => 서버연결 => 데이터 읽기 => store
                                             | commit
                                            mutation
    mutation : 동기적으로 저장 => 유일하게 state 변경이 가능
    state : 공유하는 데이터
...
export default{
    ...
    mutations:{
        ...
        SET_FOOD_FIND_DATA(state,payload){
            console.log(payload)
            state.food_find_data=payload
        }
    },
    // 서버와 연결 => 요청 담당
    /*
        ...
        2. mutaions : state 를 동기적으로 저장 (변경,삭제)
    ...
    */
    actions:{
        ...
        */
        async foodFindData({commit},{column,ss,page}){
            console.log("foodFindData 호출"+column+" "+ss+" "+page)
            await axios.get('http://localhost/food/find_vue/',{
                ...
                commit('SET_FOOD_FIND_DATA',res.data)
            })
        }
    }
}
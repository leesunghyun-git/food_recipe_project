		...
		<div class="row">
			...
			<table class="table">
				<tbody>
					...
					<tr>
						<td class="text-right"><router-link :to="{name:'home'}" class="btn btn-sm btn-primary">목록</router-link></td>
					</tr>
				</tbody>
			</table>
		</div>
...
const vo =computed(()=>store.state.foods.food_detail)
onMounted(()=>{
	store.dispatch("foods/foodDetailData",route.params.fno)
})
